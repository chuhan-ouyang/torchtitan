...
TARGET_NAME = "ncclDevKernel_AllGather_RING_LL(ncclDevKernelArgsStorage<(unsigned long)4096>)"
TARGET_PARALLELISM = "DP"

def first_endtime(csv_path: str) -> int:
    df = pd.read_csv(csv_path)
    # Important: force to Python ints to avoid overflow surprises
    ...
    row = hit.iloc[0]  # first match by file order
...
def main():
    ap = argparse.ArgumentParser(description="Sync 'other' CSV DP timing to 'base' by shifting Start (ns).")
    ...
    offset = base_end - other_end  # can be negative or positive
